...
class Solution {
    ...
    public int minFallingPathSum(int[][] A) {
        ...
        int minFallingPathSum = 0;
        for (int i = 1; i < m; i++) {
            int tempMinFallingPathSum = Integer.MAX_VALUE;
            for (int j = 0; j < n; j++) {
                int minPath = A[i-1][j];
                if (j != 0) {
                ...
                A[i][j] += minPath;
                tempMinFallingPathSum = Math.min(tempMinFallingPathSum, A[i][j]);
            }
            minFallingPathSum = tempMinFallingPathSum;
        }
