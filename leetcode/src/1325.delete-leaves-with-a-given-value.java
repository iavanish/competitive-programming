...
class Solution {
    ...
    public TreeNode removeLeafNodes(TreeNode root, int target) {
        boolean checkRoot = checkLeafNode(root, target);
        if (checkRoot) {
            return null;
        }
        return root;
    ...
    private boolean checkLeafNode(TreeNode root, int target) {
        if (root == null) {
            return false;
        }
        boolean checkLeft = checkLeafNode(root.left, target);
        if (checkLeft) {
            root.left = null;
        }
        boolean checkRight = checkLeafNode(root.right, target);
        if (checkRight) {
            root.right = null;
        }
        if (root.val == target) {
