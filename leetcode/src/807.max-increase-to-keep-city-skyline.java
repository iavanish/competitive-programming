...
class Solution {
    ...
    public int maxIncreaseKeepingSkyline(int[][] grid) {
        int n = grid.length;
        int m = grid[0].length;
        int[] rightSkyline = new int[n];
        int[] topSkyline = new int[m];

        for(int i = 0; i < n; i++) {
            for(int j = 0; j < m; j++) {
                rightSkyline[i] = Math.max(rightSkyline[i], grid[i][j]);
        ...
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < m; j++) {
                maxIncrease += Math.min(rightSkyline[i] - grid[i][j], topSkyline[j] - grid[i][j]);
            }
        }
